...
public class SpecDatabase {
    ...
    public void add(final SpecTopic topic, final String escapedTitle) {
        ...
        if (!specTopics.containsKey(topicId)) specTopics.put(topicId, new LinkedList<SpecTopic>());

        if (!specTopicsTitles.containsKey(escapedTitle)) specTopicsTitles.put(escapedTitle, new LinkedList<SpecTopic>());
    ...
    public void add(final Level level, final String escapedTitle) {
        ...
        if (!specLevels.containsKey(escapedTitle)) specLevels.put(escapedTitle, new LinkedList<Level>());

        if (specLevels.get(escapedTitle).size() > 0) level.setDuplicateId(Integer.toString(specLevels.get(escapedTitle).size()));
    ...
    public void setDatabaseDulicateIds(final Map<Integer, Set<String>> usedIdAttributes) {
        ...
        for (final String levelTitle : specLevels.keySet()) {
            ...
            for (int i = 0; i < levels.size(); i++) {
                if (i != 0) levels.get(i).setDuplicateId(Integer.toString(i));
                else levels.get(i).setDuplicateId(null);
            }
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
    protected <T extends BaseTopicWrapper<T>> List<T> getUniqueTopicsFromSpecTopics(final List<SpecTopic> specTopics) {
